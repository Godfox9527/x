...
const baseURL = "https://hy.bilivo.top";
const url = `${baseURL}/a_tasks?uuid=${uuid}`;

// 检查UUID是否已替换
if (uuid === "你的UUID") {
  $.log("❌ 请替换为你的实际UUID");
  $notify("配置错误", "PikPak自动活跃", "请在脚本中配置有效的UUID");
  $done({});
}
...
// 执行请求并处理响应（使用$httpClient兼容低版本）
$httpClient.get(request, (error, response, body) => {
  if (error) {
    $.log(`❌ 请求错误: ${error}`);
    $notify("请求错误", `UUID: ${uuid.slice(0, 8)}...`, "网络或服务器错误");
    $done({});
    return;
  }
  ...
  try {
    ...
    if (data.code === 200 && data.msg) {
      $.log(`✅ 活跃成功: ${data.msg}`);
      $notify("活跃成功", `UUID: ${uuid.slice(0, 8)}...`, data.msg);
    } else {
      $.log(`❌ 活跃失败: ${data.msg || "未知错误"}`);
      $notify("活跃失败", `UUID: ${uuid.slice(0, 8)}...`, data.msg || "未知错误");
    }
  } catch (e) {
    $.log(`❌ 解析错误: ${e.message}`);
    $notify("解析错误", `UUID: ${uuid.slice(0, 8)}...`, "服务器返回数据无效");
  }
  $done({});
});
